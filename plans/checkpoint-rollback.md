...
5. **User-facing rollback** — `/rollback` slash command (CLI + gateway) to list and restore checkpoints. Also `apollo rollback` CLI subcommand.
...
~/.apollo/checkpoints/
  {sha256(abs_dir)[:16]}/       # Shadow git repo per working directory
    ...
    APOLLO_WORKDIR              # Original dir path (for display)
...
Add to `DEFAULT_CONFIG` in `apollo_cli/config.py`:
...
**CLI subcommand** — `apollo rollback` (optional, lower priority).
...
| `apollo_cli/config.py` | Add `checkpoints` + `checkpoint_max_snapshots` to DEFAULT_CONFIG |
...
3. `apollo_cli/config.py` — config keys
