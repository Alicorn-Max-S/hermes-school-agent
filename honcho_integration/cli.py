...
Handles: apollo honcho setup | status | sessions | map | peer
"""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Apollo persistent cross-session memory.")
    print("  Config is shared with other hosts at ~/.honcho/config.json\n")

    if not _ensure_sdk_installed():
        return

    # All writes go to hosts.apollo — root keys are managed by the user
    # or the honcho CLI only.
    hosts = cfg.setdefault("hosts", {})
    apollo_host = hosts.setdefault(HOST, {})
    ...
    if not effective_key:
        ...
        print("  Run 'apollo honcho setup' again once you have a key.\n")
        return

    # Peer name
    current_peer = apollo_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        apollo_host["peerName"] = new_peer

    current_workspace = apollo_host.get("workspace") or cfg.get("workspace", "apollo")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        apollo_host["workspace"] = new_workspace

    apollo_host.setdefault("aiPeer", HOST)

    # Memory mode
    current_mode = apollo_host.get("memoryMode") or cfg.get("memoryMode", "hybrid")
    print(f"\n  Memory mode options:")
    ...
    if new_mode in ("hybrid", "honcho"):
        apollo_host["memoryMode"] = new_mode
    else:
        apollo_host["memoryMode"] = "hybrid"

    # Write frequency
    current_wf = str(apollo_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    ...
    try:
        apollo_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        apollo_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # Recall mode
    _raw_recall = apollo_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        apollo_host["recallMode"] = new_recall

    # Session strategy
    current_strat = apollo_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print(f"\n  Session strategy options:")
    print("    per-session   — new Honcho session each run, named by Apollo session ID (default)")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        apollo_host["sessionStrategy"] = new_strat

    apollo_host.setdefault("enabled", True)
    apollo_host.setdefault("saveMessages", True)
    ...
    print(f"\n  Other commands:")
    print(f"    apollo honcho status     — show full config")
    print(f"    apollo honcho mode       — show or change memory mode")
    print(f"    apollo honcho tokens     — show or set token budgets")
    print(f"    apollo honcho identity   — seed or show AI peer identity")
    print(f"    apollo honcho map <name> — map this directory to a session name\n")
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: apollo honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'apollo honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: apollo honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        apollo = hosts.get(HOST, {})
        user = apollo.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = apollo.get('aiPeer') or cfg.get('aiPeer') or HOST
        lvl = apollo.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = apollo.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print(f"\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print(f"    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print(f"    Apollo's identity in Honcho. Seed with 'apollo honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: apollo honcho mode [hybrid|honcho]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = apollo.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = apollo.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = apollo.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print(f"\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print(f"    AI-to-AI inference. Apollo asks Honcho's AI peer a question")
        ...
        print(f"\n  Set with: apollo honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'apollo honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'apollo honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    apollo honcho identity --show        — show both peer representations")
        print("    apollo honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Apollo + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Apollo\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Apollo persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  apollo honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'apollo honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'apollo honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  apollo honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'apollo'")
        print("  with Honcho configured and no prior session history.")
        print("  (Apollo calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print(f"    apollo honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'apollo honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Apollo, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'apollo honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    apollo honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  apollo honcho identity <path/to/SOUL.md>")
    ...
    print("    Apollo:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Apollo:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Apollo:   Honcho observes every message and updates representations")
    ...
    print("    Apollo:   per-session by default — each run gets its own session")
    print("              Map a custom name:  apollo honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. apollo honcho setup              — configure API key (required)")
        print("  2. apollo honcho migrate            — re-run this walkthrough")
    else:
        print("  1. apollo honcho status             — verify Honcho connection")
        print("  2. apollo                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. apollo honcho identity --show    — verify AI peer representation")
        print("  4. apollo honcho tokens             — tune context and dialectic budgets")
        print("  5. apollo honcho mode               — view or change memory mode")
    print()
