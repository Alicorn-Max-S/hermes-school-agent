...
def _resolve_memory_mode(
    ...
    Object form:  { "default": "hybrid", "apollo": "honcho", ... }
...
class HonchoClientConfig:
    ...
    # Config object form: "memoryMode": { "default": "hybrid", "apollo": "honcho" }
    ...
    # Max chars of dialectic result to inject into Apollo system prompt
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "apollo") -> HonchoClientConfig:
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Apollo session title (from /title command)
          3. per-session strategy — Apollo session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Apollo session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key:
        raise ValueError(
            ...
            "then run 'apollo honcho setup' or set HONCHO_API_KEY."
        )
