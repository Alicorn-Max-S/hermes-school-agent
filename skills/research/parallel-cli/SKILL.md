...
This is an optional third-party workflow, not a Apollo core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Apollo native `web_search` / `webscrape`, so do not prefer it by default for ordinary lookups.
...
Prefer Apollo native `web_search` / `webscrape` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Apollo native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Apollo usage patterns
...
- Do not silently choose Parallel when Apollo native tools are already sufficient.
