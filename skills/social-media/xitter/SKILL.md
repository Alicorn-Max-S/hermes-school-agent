...
version: 1.0.0
author: Siddharth Balyan + Apollo Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Apollo. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.apollo/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.apollo/.env ~/.config/x-cli/.env
```
...
- **Credential drift**: if you rotate tokens in `~/.apollo/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
