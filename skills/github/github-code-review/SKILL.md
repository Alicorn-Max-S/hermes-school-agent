...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Apollo Agent\",
    \"comments\": [
...
```bash
source ~/.apollo/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Apollo Agent. Code looks clean — good test coverage, no security concerns."
