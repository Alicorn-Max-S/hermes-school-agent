...
Architecture:
    ~/.apollo/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        APOLLO_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = Path(os.getenv("APOLLO_HOME", Path.home() / ".apollo")) / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("APOLLO_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "apollo@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Apollo Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "APOLLO_WORKDIR").write_text(
