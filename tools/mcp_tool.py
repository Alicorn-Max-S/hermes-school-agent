...
discovers their tools, and registers them into the apollo-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.apollo/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            apollo_home = os.path.expanduser(
                os.getenv(
                    "APOLLO_HOME", os.path.join(os.path.expanduser("~"), ".apollo")
                )
            )
            candidates = [
                os.path.join(apollo_home, "node", "bin", resolved_command),
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Apollo config file.
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Apollo registry schema format.
...
def discover_mcp_tools() -> List[str]:
    ...
    if all_tools:
        # Dynamically inject into all apollo-* platform toolsets
        from toolsets import TOOLSETS
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("apollo-"):
