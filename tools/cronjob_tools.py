"""
Cron job management tools for Apollo Agent.

These tools allow the agent to schedule, list, and remove automated tasks.
Only available when running via CLI (apollo-cli toolset).
...
def schedule_cronjob(
    ...
    Args:
        ...
        deliver: Where to send the output. Options:
                 ...
                 - "local": Save to local files only (~/.apollo/cron/output/)
    ...
    origin = None
    origin_platform = os.getenv("APOLLO_SESSION_PLATFORM")
    origin_chat_id = os.getenv("APOLLO_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        origin = {
            "platform": origin_platform,
            "chat_id": origin_chat_id,
            "chat_name": os.getenv("APOLLO_SESSION_CHAT_NAME"),
        }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("APOLLO_INTERACTIVE")
        or os.getenv("APOLLO_GATEWAY_SESSION")
        or os.getenv("APOLLO_EXEC_ASK")
    )
