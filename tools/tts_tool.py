...
Configuration is loaded from ~/.apollo/config.yaml under the 'tts:' key.
...
DEFAULT_OUTPUT_DIR = str(Path(os.getenv("APOLLO_HOME", Path.home() / ".apollo")) / "audio_cache")
MAX_TEXT_LENGTH = 4000


# ===========================================================================
# Config loader -- reads tts: section from ~/.apollo/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.apollo/config.yaml.
    ...
    try:
        ...
        logger.debug("apollo_cli.config not available, using default TTS config")
        return {}
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.apollo/config.yaml (tts: section).
    ...
    platform = os.getenv("APOLLO_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": "Optional custom file path to save the audio. Defaults to ~/.apollo/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
