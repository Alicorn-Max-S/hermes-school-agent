...
# 5. Symlinks the 'apollo' CLI command into ~/.local/bin
...
echo ""
echo -e "${CYAN}Apollo Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink apollo into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up apollo command..."

APOLLO_BIN="$SCRIPT_DIR/venv/bin/apollo"
mkdir -p "$HOME/.local/bin"
ln -sf "$APOLLO_BIN" "$HOME/.local/bin/apollo"
echo -e "${GREEN}✓${NC} Symlinked apollo → ~/.local/bin/apollo"
...
# ============================================================================
# Seed bundled skills into ~/.apollo/skills/
# ============================================================================

APOLLO_SKILLS_DIR="${APOLLO_HOME:-$HOME/.apollo}/skills"
mkdir -p "$APOLLO_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.apollo/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$APOLLO_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  apollo status        # Check configuration"
echo "  apollo gateway install # Install gateway service (messaging + cron)"
echo "  apollo cron list     # View scheduled jobs"
echo "  apollo doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m apollo_cli.main setup
fi
