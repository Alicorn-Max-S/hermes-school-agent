...
version: 1.0.0
author: arceus77-7, enhanced by Apollo Agent
license: MIT
metadata:
  apollo:
...
- `tmux` available for stable authenticated sessions during Apollo terminal calls (desktop app flow only)
...
### Service Account (recommended for Apollo)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.apollo/.env` (the skill will prompt for this on first load).
...
## Apollo Execution Pattern (desktop app flow)

Apollo terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/apollo-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/apollo-op.sock"
