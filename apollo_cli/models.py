...
Add, remove, or reorder entries here — both `apollo setup` and
`apollo` provider-selection will pick up the change automatically.
"""
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Apollo' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``apollo_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from apollo_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
