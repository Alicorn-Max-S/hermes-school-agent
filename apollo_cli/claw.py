"""apollo claw — OpenClaw migration commands.

Usage:
    apollo claw migrate              # Interactive migration from ~/.openclaw
    apollo claw migrate --dry-run    # Preview what would be migrated
    apollo claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
"""
...
from apollo_cli.config import get_apollo_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_apollo.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_apollo", script_path)
...
def claw_command(args):
    """Route apollo claw subcommands."""
    action = getattr(args, "claw_action", None)

    if action == "migrate":
        _cmd_migrate(args)
    else:
        print("Usage: apollo claw migrate [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Apollo")
        print()
        print("Run 'apollo claw migrate --help' for migration options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Apollo migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info(f"You can specify a custom path: apollo claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {apollo_home}")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  apollo claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
