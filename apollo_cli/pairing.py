...
Usage:
    apollo pairing list              # Show all pending + approved users
    apollo pairing approve <platform> <code>  # Approve a pairing code
    apollo pairing revoke <platform> <user_id> # Revoke user access
    apollo pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle apollo pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: apollo pairing {list|approve|revoke|clear-pending}")
        print("Run 'apollo pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  Run 'apollo pairing list' to see pending codes.\n")
