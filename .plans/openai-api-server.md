# OpenAI-Compatible API Server for Apollo Agent
...
SSE streaming. By exposing this endpoint, apollo-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  apollo-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.apollo/.env`
2. Run `apollo gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with apollo-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns apollo-agent as a model) |
...
Authorization: Bearer apollo-api-key-here
...
But apollo-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "apollo-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured apollo-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of apollo-agent's core prompt
...
Once implemented, apollo-agent works as a drop-in backend for:
