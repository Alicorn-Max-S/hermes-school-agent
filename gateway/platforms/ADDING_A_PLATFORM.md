...
Checklist for integrating a new messaging platform into the Apollo gateway.
...
```python
"apollo-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _APOLLO_CORE_TOOLS,
    "includes": []
},
```

And add it to the `apollo-gateway` composite:

```python
"apollo-gateway": {
    "includes": [..., "apollo-your-platform"]
}
```
...
## 12. Status Display (`apollo_cli/status.py`)
...
## 13. Gateway Setup Wizard (`apollo_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ apollo_cli/ toolsets.py \
  --include="*.py" -l | sort -u
