...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Apollo-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Apollo Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Apollo/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
```
                 ...
                 └───────────┬───────────┘
                             │ inherits
                 ┌───────────┴───────────┐
                 │  ApolloAgentBaseEnv    │  environments/apollo_base_env.py
                 │  - Terminal backend    │
                 │  - Tool resolution     │
                 │  - Agent loop engine   │
                 │  - ToolContext         │
       ...
       ┌─────────────────────┼─────────────────────┐
       │                     │                      │
  TerminalTestEnv     ApolloSweEnv     TerminalBench2EvalEnv
...
The apollo-agent layer (`environments/apollo_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls apollo-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `ApolloAgentLoop` and scores the result
...
Your environment inherits from `ApolloAgentBaseEnv` and implements five methods:
...
`ApolloAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as apollo-agent's main loop:
...
| **Web** | `web_search(query)`, `webscrape(urls)` |
| **Browser** | `browser_navigate(url)`, `browser_snapshot()` |
| **Generic** | `call_tool(name, args)` — escape hatch for any apollo-agent tool |
...
parser = get_parser("apollo")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `apollo`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "apollo-agent[yc-bench]"
...
```bash
python environments/apollo_swe_env/apollo_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"apollo"`, `"qwen"`, `"mistral"`)
...
```python
from environments.apollo_base_env import ApolloAgentBaseEnv, ApolloAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(ApolloAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(ApolloAgentBaseEnv):
    name = "my-env"
...
### ApolloAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which apollo toolsets to enable |
...
| `tool_call_parser` | `str` | `"apollo"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "apollo-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "apollo-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── apollo_base_env.py          # Abstract base class (ApolloAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (ApolloAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── apollo_parser.py        # Apollo/ChatML <tool_call> format
...
├── apollo_swe_env/             # SWE-bench training environment
│
