...
The ACP adapter wraps Apollo's synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
apollo acp / apollo-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.apollo/.env
  -> configure stderr logging
  -> construct ApolloACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Apollo `once`
- `allow_always` -> Apollo `always`
- reject options -> Apollo `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Apollo tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["apollo-acp"])
...
Instead it reuses Apollo's runtime resolver:

- `acp_adapter/auth.py`
- `apollo_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Apollo provider/credentials.
...
- `toolsets.py` — `apollo-acp` toolset definition
- `apollo_cli/main.py` — `apollo acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `apollo-acp` script
