...
Apollo currently supports three API execution modes:
...
Apollo wraps API requests so they can be interrupted from the CLI or gateway.
...
Apollo uses two execution strategies:
...
Apollo tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, Apollo may:
