...
description: "Catalog of bundled skills that ship with Apollo Agent"
---

# Bundled Skills Catalog

Apollo ships with a large built-in skill library copied into `~/.apollo/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `apollo-agent-spawning` | Spawn additional Apollo Agent instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate apollo process. | `autonomous-ai-agents/apollo-agent` |
...
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Apollo Agent tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| `apollo-atropos-environments` | Build, test, and debug Apollo Agent RL environments for Atropos training. Covers the ApolloAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or f… | `mlops/training/apollo-atropos-environments` |
...
| `ocr-and-documents` | Extract text from PDFs and scanned documents. Use webscrape for remote URLs, pymupdf for local text-based PDFs, marker-pdf for OCR/scanned docs. For DOCX use python-docx, for PPTX see the powerpoint skill. | `productivity/ocr-and-documents` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `arxiv` | Search and retrieve academic papers from arXiv using their free REST API. No API key needed. Search by keyword, author, category, or ID. Combine with webscrape or the ocr-and-documents skill to read full paper content. | `research/arxiv` |
...
| `duckduckgo-search` | Free web search via DuckDuckGo — text, news, images, videos. No API key needed. Use the Python DDGS library or CLI to search, then webscrape for full content. | `research/duckduckgo-search` |
