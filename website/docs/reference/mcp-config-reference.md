...
description: "Reference for Apollo Agent MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Apollo may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Apollo only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Apollo does not create an empty MCP runtime toolset for that server.
