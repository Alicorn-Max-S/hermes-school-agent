---
sidebar_position: 5
title: "Use MCP with Apollo"
description: "A practical guide to connecting MCP servers to Apollo Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Apollo

This guide shows how to actually use MCP with Apollo Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Apollo tool
- you want Apollo to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Apollo to internal APIs, databases, or company systems without modifying Apollo core

Do not use MCP when:
- a built-in Apollo tool already solves the job well
...
- Apollo remains the agent
- MCP servers contribute tools
- Apollo discovers those tools at startup or reload time
...
- Apollo banner/status should show MCP integration when configured
- ask Apollo what tools it has available
...
There are two categories of MCP-exposed functionality in Apollo:
...
2. Apollo-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Apollo will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Apollo to reason over a bounded workspace.
...
Now Apollo can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Apollo core.
...
Because Apollo now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
