...
2. **Apollo spins up** a fresh agent session with your prompt
...
- **Apollo Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.apollo/cron/output/` and you can read them anytime.
:::
...
Apollo will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Apollo Agent
```
...
Just tell Apollo what you want:
...
Apollo will create the cron job for you using the `schedule_cronjob` tool.
...
For faster briefings, tell Apollo to delegate each topic to a sub-agent:
...
Apollo will use `list_cronjobs` to find it and `remove_cronjob` to delete it.
