...
Each hook is a directory under `~/.apollo/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".apollo" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.apollo/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.apollo/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.apollo/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.apollo/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".apollo" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.apollo/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.apollo/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/apollo-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.apollo/hooks/`
