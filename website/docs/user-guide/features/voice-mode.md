...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Apollo Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Apollo Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Apollo](/docs/guides/use-voice-mode-with-apollo).

## Prerequisites

Before using voice features, make sure you have:

1. **Apollo Agent installed** — `pip install apollo-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — set `OPENAI_API_KEY`, `OPENAI_BASE_URL`, and `LLM_MODEL` in `~/.apollo/.env`
3. **A working base setup** — run `apollo` to verify the agent responds to text before enabling voice

:::tip
The `~/.apollo/` directory and default `config.yaml` are created automatically the first time you run `apollo`. You only need to create `~/.apollo/.env` manually for API keys.
:::
...
pip install apollo-agent[voice]

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install apollo-agent[messaging]

# Premium TTS (ElevenLabs)
pip install apollo-agent[tts-premium]

# Everything at once
pip install apollo-agent[all]
```
...
```bash
apollo                # Start the interactive CLI
```
...
1. Start the CLI with `apollo` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.apollo/config.yaml` (default: `ctrl+b`).
:::
...
```bash
apollo gateway        # Start the gateway (connects to configured platforms)
apollo gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@apollobyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.apollo/.env`:
```bash
...
```bash
apollo gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.apollo/.env`
...
- Review gateway logs: `tail -f ~/.apollo/logs/gateway.log`
