...
All skills live in **`~/.apollo/skills/`** — a single directory that serves as the source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.
...
```bash
apollo chat --toolsets skills -q "What skills do you have?"
apollo chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, Apollo asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `apollo setup` or `~/.apollo/.env` locally instead.

## Skill Directory Structure

```
~/.apollo/skills/                  # Single source of truth
...
```bash
apollo skills browse                              # Browse all hub skills (official first)
apollo skills browse --source official            # Browse only official optional skills
apollo skills search kubernetes                   # Search all sources
apollo skills search react --source skills-sh     # Search the skills.sh directory
apollo skills search https://mintlify.com/docs --source well-known
apollo skills inspect openai/skills/k8s           # Preview before installing
apollo skills install openai/skills/k8s           # Install with security scan
...
apollo skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
apollo skills list --source hub                   # List hub-installed skills
apollo skills check                               # Check installed hub skills for upstream updates
apollo skills update                              # Reinstall hub skills with upstream changes when needed
apollo skills audit                               # Re-scan all hub skills for security
apollo skills uninstall k8s                       # Remove a hub skill
apollo skills publish skills/my-skill --to github --repo owner/repo
apollo skills snapshot export setup.json          # Export skill config
apollo skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Apollo. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `apollo skills search <query> --source skills-sh`. Apollo resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Apollo currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Apollo repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Apollo can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
apollo skills search https://mintlify.com/docs --source well-known
apollo skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
apollo skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Apollo can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
apollo skills tap add myorg/skills-repo
```
...
- Apollo source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Apollo supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Apollo source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Apollo can search and convert agent entries from LobeHub's public catalog into installable Apollo skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Apollo source id: `lobehub`
...
`apollo skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Apollo | Always trusted |
...
```bash
apollo skills check          # Report which installed hub skills changed upstream
apollo skills update         # Reinstall only the skills with updates available
apollo skills update react   # Update one specific installed hub skill
```
