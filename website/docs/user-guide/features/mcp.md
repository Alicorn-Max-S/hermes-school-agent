...
description: "Connect Apollo Agent to external tool servers via MCP — and control exactly which MCP tools Apollo loads"
---

# MCP (Model Context Protocol)

MCP lets Apollo Agent connect to external tool servers so the agent can use tools that live outside Apollo itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Apollo to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Apollo tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Apollo to see
...
2. Add an MCP server to `~/.apollo/config.yaml`:
...
4. Ask Apollo to use the MCP-backed capability.
...
Apollo will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Apollo connects to directly.
...
- you do not want Apollo spawning a local subprocess for that integration

## Basic configuration reference

Apollo reads MCP config from `~/.apollo/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Apollo skips the server entirely |
...
## How Apollo registers MCP tools

Apollo prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Apollo sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Apollo also registers utility tools around MCP resources and prompts:
...
- Apollo only registers resource utilities if the MCP session actually supports resource operations
- Apollo only registers prompt utilities if the MCP session actually supports prompt operations
...
You can now control which tools each MCP server contributes to Apollo.
...
If `enabled: false`, Apollo skips the server completely and does not even attempt a connection.
...
You can also separately disable Apollo-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Apollo does not create an empty runtime MCP toolset for that server.
...
Apollo discovers MCP servers at startup and registers their tools into the normal tool registry.
...
For stdio servers, Apollo does not blindly pass your full shell environment.
...
Then verify your config and restart Apollo.
...
Because Apollo now only registers those wrappers when both are true:
