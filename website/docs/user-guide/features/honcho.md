...
[Honcho](https://honcho.dev) is an AI-native memory system that gives Apollo persistent, cross-session understanding of users. While Apollo has built-in memory (`MEMORY.md` and `USER.md`), Honcho adds a deeper layer of **user modeling** — learning preferences, goals, communication style, and context across conversations via a dual-peer architecture where both the user and the AI build representations over time.

## Works Alongside Built-in Memory

Apollo has two memory systems that can work together or be configured separately. In `hybrid` mode (the default), both run side by side — Honcho adds cross-session user modeling while local files handle agent-level notes.

| Feature | Built-in Memory | Honcho Memory |
|---------|----------------|---------------|
| Storage | Local files (`~/.apollo/memories/`) | Cloud-hosted Honcho API |
...
`apiKey` lives at the root because it is a shared credential across all Honcho-enabled tools. All other settings are scoped under `hosts.apollo`. The `apollo honcho setup` wizard writes this structure automatically.

Or set the API key as an environment variable:

```bash
apollo config set HONCHO_API_KEY your-key
```
...
Settings are scoped to `hosts.apollo` and fall back to root-level globals when the host field is absent. Root-level keys are managed by the user or the honcho CLI -- Apollo only writes to its own host block (except `apiKey`, which is a shared credential at root).
...
**Host-level (`hosts.apollo`)**

| Field | Default | Description |
|-------|---------|-------------|
| `workspace` | `"apollo"` | Workspace identifier |
| `peerName` | *(derived)* | Your identity name for user modeling |
| `aiPeer` | `"apollo"` | AI assistant identity name |
...
All host-level fields fall back to the equivalent root-level key if not set under `hosts.apollo`. Existing configs with settings at root level continue to work.
...
### Apollo Config (`~/.apollo/config.yaml`)
...
```
apollo honcho setup                        # Interactive setup wizard
apollo honcho status                       # Show config and connection status
...
apollo honcho tokens --context N           # Set context token cap
apollo honcho tokens --dialectic N         # Set dialectic char cap
apollo honcho identity                     # Show AI peer identity
apollo honcho identity <file>              # Seed AI peer identity from file (SOUL.md, etc.)
apollo honcho migrate                      # Migration guide: OpenClaw → Apollo + Honcho
```

### Doctor Integration

`apollo doctor` includes a Honcho section that validates config, API key, and connection status.
...
```bash
apollo honcho identity ~/.apollo/SOUL.md
```
