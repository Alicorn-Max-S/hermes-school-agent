...
Apollo Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Apollo instance and is loaded from `APOLLO_HOME` only.
...
| **SOUL.md** | Global personality and tone customization for this Apollo instance | `APOLLO_HOME/SOUL.md` only |
...
Apollo walks the directory tree starting from the working directory and loads **all** `AGENTS.md` files found, sorted by depth. This supports monorepo-style setups:
...
- `~/.apollo/SOUL.md`
- or `$APOLLO_HOME/SOUL.md` if you run Apollo with a custom home directory

Important details:

- Apollo seeds a default `SOUL.md` automatically if one does not exist yet
- Apollo loads `SOUL.md` only from `APOLLO_HOME`
- Apollo does not probe the working directory for `SOUL.md`
...
Apollo is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root, they're loaded alongside AGENTS.md.

This means your existing Cursor conventions automatically apply when using Apollo.
