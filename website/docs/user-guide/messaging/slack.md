...
sidebar_position: 4
title: "Slack"
description: "Set up Apollo Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Apollo Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Apollo instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Apollo uses the modern
...
4. Enter an app name (e.g., "Apollo Agent") and select your workspace
5. Click **Create App**
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `apollo-socket` (the name doesn't matter)
...
Apollo uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
apollo gateway setup    # Select Slack when prompted
```
...
Understanding how Apollo behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Apollo Agent what time is it?`) |
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Apollo will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Apollo Agent`).

---

## Voice Messages

Apollo supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Apollo Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Apollo Agent` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Apollo Agent`)
...
- Tokens should be stored in `~/.apollo/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Apollo config directory
