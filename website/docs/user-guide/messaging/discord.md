...
sidebar_position: 3
title: "Discord"
description: "Set up Apollo Agent as a Discord bot"
---

# Discord Setup

Apollo Agent integrates with Discord as a bot, letting you chat with your AI assistant through direct messages or server channels. The bot receives your messages, processes them through the Apollo Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, voice messages, file attachments, and slash commands.
...
3. Enter a name for your application (e.g., "Apollo Agent") and accept the Developer Terms of Service.
4. Click **Create**.
...
The bot token is the credential Apollo Agent uses to log in as your bot. Still on the **Bot** page:
...
After authorizing, the bot will appear in your server's member list (it will show as offline until you start the Apollo gateway).

## Step 7: Find Your Discord User ID

Apollo Agent uses your Discord User ID to control who can interact with the bot. To find it:
...
:::tip
You can run `apollo gateway` in the background or as a systemd service for persistent operation. See the deployment docs for details.
:::
...
Apollo Agent supports Discord voice messages:
...
- **Discord voice channels**: Apollo can also join a voice channel, listen to users speaking, and talk back in the channel.

For the full setup and operational guide, see:
- [Voice Mode](/docs/user-guide/features/voice-mode)
- [Use Voice Mode with Apollo](/docs/guides/use-voice-mode-with-apollo)
...
**Cause**: The Apollo gateway isn't running, or the token is incorrect.

**Fix**: Check that `apollo gateway` is running. Verify `DISCORD_BOT_TOKEN` in your `.env` file. If you recently reset the token, update it.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `DISCORD_ALLOWED_USERS`.

**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.apollo/.env` and restart the gateway.
...
For more information on securing your Apollo Agent deployment, see the [Security Guide](../security.md).
