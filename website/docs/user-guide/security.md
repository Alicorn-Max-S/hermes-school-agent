...
Apollo Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Apollo checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
The `APOLLO_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.apollo/config.yaml`:
...
:::tip
Use `apollo config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Apollo controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.apollo/.env`:
...
For more flexible authorization, Apollo includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `apollo pairing approve <platform> <code>` on the CLI
...
apollo pairing approve telegram ABC12DEF

# Revoke a user's access
apollo pairing revoke telegram 123456789

# Clear all pending codes
apollo pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.apollo/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Apollo applies strict security hardening to every container.
...
Container resources are configurable in `~/.apollo/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.apollo/sandboxes/docker/<task_id>/`
...
4. **Store secrets securely** — keep API keys in `~/.apollo/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.apollo/logs/` for unauthorized access attempts
10. **Keep updated** — run `apollo update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.apollo/.env
