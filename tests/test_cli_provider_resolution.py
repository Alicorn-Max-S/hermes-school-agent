...
from apollo_cli.auth import AuthError
from apollo_cli import main as apollo_main
...
def test_apollo_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in ApolloCLI.__init__")

    monkeypatch.setattr("apollo_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("apollo_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.ApolloCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("apollo_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("apollo_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    monkeypatch.setenv("APOLLO_INFERENCE_PROVIDER", "openrouter")
    ...
    shell = cli.ApolloCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("apollo_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "apollo_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.ApolloCLI(compact=True, max_turns=1)
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.ApolloCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.ApolloCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "apollo_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("apollo_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("apollo_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("apollo_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("apollo_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(apollo_main, "_prompt_provider_choice", lambda choices: len(choices) - 1)

    apollo_main.cmd_model(SimpleNamespace())
