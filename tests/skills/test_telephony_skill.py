...
def test_save_twilio_writes_env_and_state(tmp_path: Path, monkeypatch):
    mod = load_module()
    monkeypatch.setenv("APOLLO_HOME", str(tmp_path / ".apollo"))
    ...
    env_text = (tmp_path / ".apollo" / ".env").read_text(encoding="utf-8")
    state = json.loads((tmp_path / ".apollo" / "telephony_state.json").read_text(encoding="utf-8"))
...
def test_diagnose_includes_decision_tree_and_saved_state(tmp_path: Path, monkeypatch):
    ...
    mod._save_state(
        ...
        apollo_home / "telephony_state.json",
    )
    (apollo_home / ".env").parent.mkdir(parents=True, exist_ok=True)
    (apollo_home / ".env").write_text(
