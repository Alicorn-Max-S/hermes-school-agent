...
class TestBuildContextFilesPrompt:
    ...
    def test_loads_soul_md_from_apollo_home_only(self, tmp_path, monkeypatch):
        monkeypatch.setenv("APOLLO_HOME", str(tmp_path / "apollo_home"))
        apollo_home = tmp_path / "apollo_home"
        apollo_home.mkdir()
        (apollo_home / "SOUL.md").write_text("Be concise and friendly.", encoding="utf-8")
    ...
    def test_empty_soul_md_adds_nothing(self, tmp_path, monkeypatch):
        ...
        apollo_home = tmp_path / "apollo_home"
        apollo_home.mkdir()
        (apollo_home / "SOUL.md").write_text("\n\n", encoding="utf-8")
        result = build_context_files_prompt(cwd=str(tmp_path))
        assert result == ""
