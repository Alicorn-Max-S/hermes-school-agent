"""Tests that provider selection via `apollo model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated APOLLO_HOME with a minimal string-format config."""
    home = tmp_path / "apollo"
    home.mkdir()
    ...
    env_file = home / ".env"
    env_file.write_text("")
    monkeypatch.setenv("APOLLO_HOME", str(home))
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from apollo_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from apollo_cli.main import _model_flow_api_key_provider
        ...
        with patch("apollo_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("apollo_cli.auth.deactivate_provider"), \
