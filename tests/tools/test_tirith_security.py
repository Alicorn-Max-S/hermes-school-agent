...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._apollo_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_apollo_bin(self):
        """After _INSTALL_FAILED, manual install in APOLLO_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        apollo_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(apollo_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(apollo_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._apollo_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == apollo_bin
            assert _tirith_mod._resolved_path == apollo_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# APOLLO_HOME isolation
# ---------------------------------------------------------------------------

class TestApolloHomeIsolation:
    def test_apollo_bin_dir_respects_apollo_home(self):
        """_apollo_bin_dir must use APOLLO_HOME, not hardcoded ~/.apollo."""
        from tools.tirith_security import _apollo_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"APOLLO_HOME": tmpdir}):
            result = _apollo_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_apollo_home(self):
        """_failure_marker_path must use APOLLO_HOME, not hardcoded ~/.apollo."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"APOLLO_HOME": "/custom/apollo"}):
            result = _failure_marker_path()
        assert result == "/custom/apollo/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets APOLLO_HOME; verify it's active."""
        apollo_home = os.getenv("APOLLO_HOME")
        assert apollo_home is not None, "APOLLO_HOME should be set by conftest"
        assert "apollo_test" in apollo_home, "Should point to test temp dir"

    def test_get_apollo_home_fallback(self):
        """Without APOLLO_HOME set, falls back to ~/.apollo."""
        from tools.tirith_security import _get_apollo_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove APOLLO_HOME entirely
            os.environ.pop("APOLLO_HOME", None)
            result = _get_apollo_home()
        assert result == os.path.join(os.path.expanduser("~"), ".apollo")
