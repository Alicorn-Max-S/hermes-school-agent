"""Tests for acp_adapter.server — ApolloACPAgent ACP server."""
...
from acp_adapter.server import ApolloACPAgent, APOLLO_VERSION
...
def agent(mock_manager):
    """ApolloACPAgent backed by a mock session manager."""
    return ApolloACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "apollo-agent"
        assert resp.agent_info.version == APOLLO_VERSION
