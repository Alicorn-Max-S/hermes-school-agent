...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from apollo_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        apollo_home = tmp_path / ".apollo"
        apollo_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: apollo_home / ".env")
        monkeypatch.setattr(config_module, "get_apollo_home", lambda: apollo_home)
        from apollo_cli.main import _has_any_provider_configured
