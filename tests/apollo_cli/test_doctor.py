...
import apollo_cli.doctor as doctor
import apollo_cli.gateway as gateway_cli
from apollo_cli import doctor as doctor_mod
from apollo_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "APOLLO_HOME", apollo_home)
    monkeypatch.delenv("APOLLO_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("APOLLO_INTERACTIVE")
        raise SystemExit(0)
...
def test_check_gateway_service_linger_warns_when_disabled(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "apollo-gateway.service"
