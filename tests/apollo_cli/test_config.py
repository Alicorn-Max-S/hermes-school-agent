"""Tests for apollo_cli configuration management."""
...
class TestGetApolloHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("APOLLO_HOME", None)
            home = get_apollo_home()
            assert home == Path.home() / ".apollo"

    def test_env_override(self):
        with patch.dict(os.environ, {"APOLLO_HOME": "/custom/path"}):
            home = get_apollo_home()
            assert home == Path("/custom/path")


class TestEnsureApolloHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"APOLLO_HOME": str(tmp_path)}, clear=False):
