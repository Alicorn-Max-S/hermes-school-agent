...
class TestNonInteractiveSetup:
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("apollo_cli.setup.ensure_apollo_home"),
            patch("apollo_cli.setup.load_config", return_value={}),
            patch("apollo_cli.setup.get_apollo_home", return_value="/tmp/.apollo"),
            patch("apollo_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "apollo config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `apollo` should not prompt for input when no provider exists and stdin is headless."""
        from apollo_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("apollo_cli.main._has_any_provider_configured", return_value=False),
            patch("apollo_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
