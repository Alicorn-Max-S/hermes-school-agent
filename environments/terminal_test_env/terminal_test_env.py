...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Apollo Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Apollo Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Apollo Agent",
    },
    {
...
class TerminalTestEnv(ApolloAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/apollo-agent/.env.
        """
