"""
ApolloAgentBaseEnv -- Abstract Base Environment for Apollo-Agent + Atropos

Provides the Atropos integration plumbing that all apollo-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ApolloAgentLoop
...
# Ensure the apollo-agent repo root is on sys.path so that imports like
...
# Load API keys from apollo-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, ApolloAgentLoop
from environments.tool_context import ToolContext

# Import apollo-agent toolset infrastructure
...
class ApolloAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for apollo-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of apollo toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: apollo, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class ApolloAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for apollo-agent Atropos integration.
    ...
    name: Optional[str] = "apollo-agent"
    env_config_cls = ApolloAgentEnvConfig

    def __init__(
        self,
        config: ApolloAgentEnvConfig,
        ...
        # Set terminal environment variables so apollo tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY apollo-agent tool (terminal, file, web,
