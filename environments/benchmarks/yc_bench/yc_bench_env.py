...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ApolloAgentLoop with terminal tool only
...
Requires: pip install apollo-agent[yc-bench]
"""
...
class YCBenchEvalConfig(ApolloAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ApolloAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ApolloAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ApolloAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ApolloAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ApolloAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the ApolloAgentLoop
            # ----------------------------------------------------------
